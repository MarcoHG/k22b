...
// This micro has no FSL_FEATURE_FLASH_HAS_PFLASH_BLOCK_SWAP
*/
...
/*!
 * @brief Flash entry point
 *
 * Initialize pflash structures
 */
bool pflash_write(uint32_t *pData, uint16_t nbrWords)
{
  ...
  ///
  /* Test pflash basic operation only if flash is unsecured. */
  if (kFTFx_SecurityStateNotSecure == securityStatus)
  {
    ...
    result = FLASH_Program(&s_flashDriver, destAdrss, pData, nbrWords*4);
    if (kStatus_FTFx_Success != result)
    {
    	return false;
    }
    ...
    result = FLASH_VerifyProgram(&s_flashDriver, destAdrss, nbrWords*4, (uint8_t *)pData, kFTFx_MarginValueUser,
                                 &failAddr, &failDat);
    if (kStatus_FTFx_Success != result)
    {
    	return false;
...
bool	pflash_read(uint32_t *pData, uint16_t nbrWords)
{
	...
	if (kFTFx_SecurityStateNotSecure == securityStatus)
	{
		...
		// debug:	PRINTF("\r\n Reading a sector of flash");
		destAdrss = pflashBlockBase + (pflashTotalSize - (SECTOR_INDEX_FROM_END * pflashSectorSize));

		/* Verify programming by reading back from flash directly*/
		for (uint32_t i = 0; i < nbrWords; i++)
		{
			pData[i] = *(volatile uint32_t *)(destAdrss + i * 4);
		}
	}
	return true;
}
